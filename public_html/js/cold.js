/* 
 * Larissa: Skript för kalla drycker
 */
var posts = [];
var food = [];
...
function getData() {
    const url = "http://localhost:3000/api/cold";
...
function displayTable()
{
    ...
    if (food.length > 0)
    {
        str = "<table id='displaytable'><tr><th>id</th><th>Namn</th><th>Kategori</th><th>Typ</th><th>Pris, SEK</th><th>Innehåll</th><th>Edit</th><th>Delete</th></tr>";

        for (var i = 0; i < food.length; i++) {
            foodstr += "<tr>";
            foodstr += "<td>" + food[i].id + "</td><td>" + food[i].name + "</td><td>" + food[i].category + "</td><td>" + food[i].type + "</td><td>" + food[i].price + "</td><td>" + food[i].ingredients + "</td>";
...
function deleteFood(foodid)
{
    ...
    const url = "http://localhost:3000/protected/cold/";
...
function editFood(food)
{
    ...
    var foodstr = "<tr><th>Namn </th><td><input type='text' id='name' required value='" + food.name + "'></td>"
            + "<th>Kategori </th><td><input type='text' id='category' required value='" + food.category + "'></td></tr><tr><th>Typ</th><td><input type='text' id='type' required value='" + food.type
...
{
    const url = "http://localhost:3000/protected/beverage";
    // Får token från localStorage
    let token = localStorage.getItem('token');
